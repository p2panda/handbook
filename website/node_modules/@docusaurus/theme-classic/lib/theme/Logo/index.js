...
const Logo = props => {
  ...
  const themedImage = <_ThemedImage.default sources={sources} height={logo.height} width={logo.width} alt={logo.alt || navbarTitle || title} />;
  return <_Link.default to={logoLink} {...propsRest} {...logo.target && {
    target: logo.target
  }}>
      {logo.src && (imageClassName ? <div className={imageClassName}>{themedImage}</div> : <>{themedImage}</>)}
