...
async function cleanCssMinify(input, inputSourceMap, minimizerOptions) {
  ...
  const sourceMap = result.sourceMap && result.sourceMap.toJSON(); // workaround for source maps on windows

  if (sourceMap) {
    // eslint-disable-next-line global-require
    const isWindowsPathSep = require("path").sep === "\\";
    sourceMap.sources = sourceMap.sources.map(item => isWindowsPathSep ? item.replace(/\\/g, "/") : item);
  }
