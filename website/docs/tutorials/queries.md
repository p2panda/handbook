...
:::info This isn't a GraphQL tutorial

GraphQL is a query language, it's not in the scope of this tutorial to introduce it from scratch,
but if you want a primer you can head over to the [GraphQL homepage](https://graphql.org/) to learn more.
...
**ACTION**: Ok, now you need to start your `aquadoggo`.
...
runs. If you stop it at any point, you'll have to repeat this step before continuing with the
tutorial again.

:::

If it's not listening on the default http address for any reason (it should be) then please update `ENDPOINT` in the code below.
...
all the seed data we're gunna use in the rest of this tutorial.

:::info GraphQL playground

At this point I have to hold up my hands and admit than some parts of this tutorial tooling are
...
It's _more fun_ to do it inline in fancy little code editors though!! Right??

:::

## Step 2: Filtering collection queries

So at this point we have our schema and seed data deployed, the node has automatically rebuilt the
GraphQL api in the background and we can start querying data.
...
and see the returned results change.
...
We want to be able to add vocabulary documents to our custom `schema_sets` documents so that we
...
want to add it to. Then click the "Add" button. You should see a message pop up saying "Added
vocabulary to study set: ...". Well done, you created a relation between documents! You can add a
few more so our later queries are a little more interesting.
